...
| `grafo` | Valida dependencias, detecta ciclos y determina qué actividades están listas. |
| `scheduler` | Administra procesos, pipes, concurrencia y señales. |
...
Este comando compila todos los archivos fuente y genera el ejecutable:

```text
planificador
```

Durante la compilación también se generan archivos objeto (`.o`). Para eliminar estos archivos y el ejecutable se utiliza:
...
| `[ABORTADA]` | Magenta | Una actividad pendiente o en ejecución fue abortada. |
...
El proyecto se divide en archivos fuente y de cabecera para mantener separadas las responsabilidades del programa. De esta manera, `main.cpp` coordina la ejecución general, mientras los demás módulos se encargan del procesamiento del archivo de entrada, la validación del grafo, la planificación de procesos y la presentación de mensajes en la terminal.

### Planificación basada en procesos

Cada actividad se ejecuta mediante un proceso hijo creado con `fork()`. El proceso padre controla la cantidad de actividades activas y respeta el límite de concurrencia indicado por `K`.
